...
video.volume = 0.3; //client events

video.onplay = () => {
  if (!wasRecentlyUpdated) {
    socket.emit('client.video:play', {
      user: socket.id
    });
...
video.onpause = () => {
  if (!wasRecentlyUpdated) {
    socket.emit('client.video:pause', {
      user: socket.id
    });
...
video.onseeked = e => {
  if (!wasRecentlyUpdated && e.target.currentTime >= 1) {
    socket.emit('client.video:seeked', {
...
socket.on('server.video:currentTime', data => {
  socket.emit('client.video:currentTime', {
